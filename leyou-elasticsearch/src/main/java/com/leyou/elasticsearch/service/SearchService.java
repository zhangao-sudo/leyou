...
import com.leyou.elasticsearch.client.SpectionClient;
...
/**
 * Created by Zhang Ao on 2020/1/24 17:18
...
public class SearchService {
    ...
    public Goods buildGoods(Spu spu) throws IOException {
        ...
        // 查询分类名称
        List<String> names = this.categoryClient.findnamebyId(Arrays.asList(spu.getCid1(), spu.getCid2(), spu.getCid3()));

        // 查询spu下的所有sku
        List<Sku> skus = this.goodsClient.findSkubyid(spu.getId());
        ...
        // 查询出所有的搜索规格参数
        List<SpecParam> params = this.specificationClient.queryParams(null, spu.getCid3(),true);
        // 查询spuDetail。获取规格参数值
        SpuDtail spuDetail = this.goodsClient.findSpudetailbyid(spu.getId());
        // 获取通用的规格参数
    ...
    public SearchResult search(SearchRequest request) {
        ...
        // 1、对key进行全文检索查询
       // QueryBuilder queryBuilder1 = QueryBuilders.matchQuery("all", key).operator(Operator.AND);
        ...
        // 执行搜索，获取搜索的结果集
        AggregatedPage<Goods> goodsPage = (AggregatedPage<Goods>)this.ItemResposity.search(queryBuilder.build());
    ...
    private BoolQueryBuilder buildBoolQueryBuilder(SearchRequest request) {
        ...
        //判断过滤条件
        if(CollectionUtils.isEmpty(request.getMaxFilter()))
        {
              return  boolQueryBuilder;
        }
        request.getMaxFilter().entrySet().forEach(entry -> {
    ...
    private List<Map<String, Object>> getParamAggResult(Long id, QueryBuilder basicQuery) {
        ...
        // 查询要聚合的规格参数
        List<SpecParam> params = this.specificationClient.queryParams(null, id, true);
        // 添加聚合
        params.forEach(param -> {
    ...
    private List<Brand> getBrandAggResult(Aggregation aggregation) {
        ...
        buckets.forEach(bucket -> {
            Brand brand = this.brandClient.findBrandbyid(bucket.getKeyAsNumber().longValue());
            brands.add(brand);
        });
        return brands;
    }
    ...
    public void createIndex(Long id) throws IOException {
        ...
        // 保存数据到索引库
        this.ItemResposity.save(goods);
    }

    public void deleteIndex(Long id) {
        this.ItemResposity.deleteById(id);
    }
}
